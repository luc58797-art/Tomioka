...
function attack() {
    ...
    projectiles.push({
        ...
        vx: Math.cos(angle) * (8.5 + player.technique * .35),
        vy: Math.sin(angle) * (8.5 + player.technique * .35),

        r: 10 + player.technique * .8,
    ...
    player.attackCooldown =
        Math.max(8, 17 - player.technique);

    addParticle(
        player.x + Math.cos(angle) * 25,
        player.y + Math.sin(angle) * 25,
        tech.color,
        12 + player.technique * 3,
        3.5 + player.technique * .3
    );

    playTone(
        430 + player.technique * 70,
        .055,
        "sine",
        .05
    );
